...
openvr_camera::openvr_camera(vr::IVRSystem *hmd)
    : hmd(hmd), tracked_camera(nullptr),
      tracked_camera_handle(INVALID_TRACKED_CAMERA_HANDLE), query_rate(60.0f) {
...
bool openvr_camera::start_impl() { // Allocate for camera frame buffer
  ...
  if (tracked_camera->GetCameraFrameSize(
          ...
          vr::VRTrackedCameraFrameType_Distorted, &frame_width, &frame_height,
  ...
  if (camera_framebuffer_size &&
      camera_framebuffer_size !=
          frame.size() * sizeof(decltype(frame)::value_type)) {
    switch (frame_format) {
  ...
  //    vr::VRTrackedCameraFrameType_Undistorted, &focal_length, &center);
...
bool openvr_camera::query() {
  ...
  vr::EVRTrackedCameraError camera_error =
      tracked_camera->GetVideoStreamFrameBuffer(
          tracked_camera_handle, vr::VRTrackedCameraFrameType_Undistorted,
  ...
  camera_error = tracked_camera->GetVideoStreamFrameBuffer(
      tracked_camera_handle, vr::VRTrackedCameraFrameType_Undistorted,
      frame.data(),
      static_cast<uint32_t>(frame.size() * sizeof(decltype(frame)::value_type)),
  ...
  vr::VRTrackedCameraFrameType_Undistorted, &tex, &frame_header,
